...
にわとりが1匹だけだとさみしいです。``||loops:くりかえし○○回||``ブロックを取得して、**100**回に設定します。``||mobs:spawn animal||``を``||loops:くりかえし○○回||`` **100**の中にドラッグして、それから``||loops:くりかえし○○回||`` 100を**chicken**の``||player:チャットコマンド||``コマンドの中にドラッグします。
